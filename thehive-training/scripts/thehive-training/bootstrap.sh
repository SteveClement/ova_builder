...
echo 'deb http://ppa.launchpad.net/webupd8team/java/ubuntu trusty main' |  tee -a /etc/apt/sources.list.d/java.list 
...
sed  -r  "s/^#(play.crypto.secret=).*/\1\"$random\"/" /etc/cortex/application.conf|  tee /etc/cortex/application.conf > /dev/null
...
cp 
random=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 64 | head -n 1)
sed  -r  "s/^#(play.crypto.secret=).*/\1\"$random\"/" /etc/thehive/application.conf|  tee /etc/thehive/application.conf > /dev/null

cortex_name="LOCAL CORTEX"
cortex_url="http://127.0.0.1:9999"
# sed -i '/^cortex\ \{$/a "$cortex_name"\ \{\n\ \ url=\"$cortex_url\"' /etc/cortex/apcortex/application.conf
